...
def test_bg_fetch():
    ...
    with requests_mock.Mocker() as mock:
        ...
        with open(local_test_path / "projects_2015.xlsx") as test_xls:
            assert(test_xls.read() == 'resp')


def test_bg_export():

    with open(Path("./tests") / "bg_projects_sample.xlsx", mode='rb') as sample_xls:
        sample_content = sample_xls.read()
    with open(Path("./tests") / "bg_state_aids.xlsx", mode='rb') as state_aids_xls:
    ...
    with requests_mock.Mocker() as mock:
        ...
        assert(os.path.exists(local_test_path))
        with open(local_test_path / "projects_2015.xlsx", mode='rb') as test_xls:
